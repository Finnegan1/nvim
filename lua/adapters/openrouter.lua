...
local function get_models(self)
  ...
  if not ok then
    log:warn('Could not get OpenRouter models from %s%s. Using fallback models.\nError: %s',
      url, models_endpoint, response)
    return FALLBACK_MODELS
  end

  ok, json = pcall(vim.json.decode, response.body)
  if not ok then
    log:warn('Could not parse response from %s%s. Using fallback models.', url, models_endpoint)
    return FALLBACK_MODELS
  end
    ...
    if params.tools then
      _cached_models[model.id] = {
        opts = {
          stream = true,
          has_tools = true,
          has_vision = inputs.image or false,
          can_reason = params.reasoning or false,
        }
      }
    end
  end

  -- Merge fallback models to ensure they're available
  for model_id, model_config in pairs(FALLBACK_MODELS) do
    if not _cached_models[model_id] then
      _cached_models[model_id] = model_config
    end
  end
